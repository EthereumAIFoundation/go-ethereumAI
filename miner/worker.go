...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, eai Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		...
		chain:          eai.BlockChain(),
		proc:           eai.BlockChain().Validator(),
	...
	worker.chainHeadSub = eai.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = eai.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.eai.TxPool().Pending()
	if err != nil {
