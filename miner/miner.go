...
// Package miner implements EthereumAI block creation and mining.
package miner
...
func New(eai Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		eai:      eai,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, eai, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(eai.BlockChain(), engine))
	go miner.update()
