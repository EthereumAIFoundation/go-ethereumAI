...
import (
	...
	"github.com/ethereumai/go-ethereumai/les/flowcontrol"
...
func NewLesServer(eai *eai.EthereumAI, config *eai.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(eai.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, eai.EventMux(), eai.Engine(), newPeerSet(), eai.BlockChain(), eai.TxPool(), eai.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(eai.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(eai.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(eai.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(eai.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(eai.ChainDb())
	return srv, nil
}
...
func newCostStats(db eaidb.Database) *requestCostStats {
