...
#
# The cluster contains a bootnode, a geai node and multiple swarm nodes, with
...
# static geai configuration
GEAI_IP="192.168.33.3"
GEAI_RPC_PORT="9545"
GEAI_RPC_URL="http://${GEAI_IP}:${GEAI_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the geai and
# swarm nodes
start_bootnode() {
...
# start_geai_node starts a geai node with --datadir pointing at <base-dir>/geai
# and a single, unlocked account with password "geai"
start_geai_node() {
  local dir="${base_dir}/geai"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get geai account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GEAI_IP}"
    --rpcport   "${GEAI_RPC_PORT}"
    --verbosity "6"
  )

  start_node "geai" "${GEAI_IP}" "$(which geai)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the veai pair
  local veai0="veai${name}0"
  local veai1="veai${name}1"
  ip link add name "${veai0}" type veai peer name "${veai1}"

  # add one end to the bridge
  ip link set dev "${veai0}" master "${BRIDGE_NAME}"
  ip link set dev "${veai0}" up

  # add the other end to the namespace, rename it eai0 and give it the ip
  ip link set dev "${veai1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${veai1}" name "eai0"
  ip netns exec "${name}" ip link set dev "eai0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "eai0"
}
...
create_account() {
  ...
  geai --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
