...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout eai_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for EaiSubscribe requests
}
...
// EaiSubscribe registers a subscripion under the "eai" namespace.
func (c *Client) EaiSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "eai", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. EaiSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through EaiSubscribe.
