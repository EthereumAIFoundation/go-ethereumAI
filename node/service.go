...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (eaidb.Database, error) {
	if ctx.config.DataDir == "" {
		return eaidb.NewMemDatabase(), nil
	}
	db, err := eaidb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
