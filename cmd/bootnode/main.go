...
// bootnode runs a bootstrap node for the EthereumAI Discovery Protocol.
package main
...
func main() {
	...
	if natm != nil {
		if !realaddr.IP.IsLoopback() {
			go nat.Map(natm, nil, "udp", realaddr.Port, realaddr.Port, "ethereumai discovery")
		}
	...
	if *runv5 {
		if _, err := discv5.ListenUDP(nodeKey, conn, realaddr, ""); err != nil {
			utils.Fatalf("%v", err)
		}
	} else {
		cfg := discover.Config{
