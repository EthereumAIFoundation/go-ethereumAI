...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.eaistats == "" {
		log.Error("No eaistats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			...
			infos = &nodeInfos{port: 40404, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if w.conf.Genesis.Config.Eaiash != nil && !boot {
		fmt.Println()
		if infos.eaiashdir == "" {
			fmt.Printf("Where should the eaiash mining DAGs be stored on the remote machine?\n")
			infos.eaiashdir = w.readString()
		} else {
			fmt.Printf("Where should the eaiash mining DAGs be stored on the remote machine? (default = %s)\n", infos.eaiashdir)
			infos.eaiashdir = w.readDefaultString(infos.eaiashdir)
		}
	}
	...
	if infos.eaistats == "" {
		...
		infos.eaistats = w.readString() + ":" + w.conf.eaistats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.eaistats)
		infos.eaistats = w.readDefaultString(infos.eaistats) + ":" + w.conf.eaistats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Eaiash != nil {
			// Eaiash based miners only need an etheraibase to mine against
			fmt.Println()
			if infos.etheraibase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.etheraibase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.etheraibase)
				infos.etheraibase = w.readDefaultAddress(common.HexToAddress(infos.etheraibase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy EthereumAI node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
