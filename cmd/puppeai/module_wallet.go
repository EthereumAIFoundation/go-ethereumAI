...
FROM puppeai/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'geai --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'geai --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --eaistats \'{{.Eaistats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppeaiNetworkID/{{.NetworkID}}/g' dist/js/etheraiwallet-master.js && \
	sed -i 's/PuppeaiNetwork/{{.Network}}/g'     dist/js/etheraiwallet-master.js && \
	sed -i 's/PuppeaiDenom/{{.Denom}}/g'         dist/js/etheraiwallet-master.js && \
	sed -i 's/PuppeaiHost/{{.Host}}/g'           dist/js/etheraiwallet-master.js && \
	sed -i 's/PuppeaiRPCPort/{{.RPCPort}}/g'     dist/js/etheraiwallet-master.js
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	stats := &walletInfos{
		datadir:  infos.volumes["/root/.ethereumai"],
		nodePort: nodePort,
		rpcPort:  rpcPort,
		webHost:  host,
		webPort:  webPort,
