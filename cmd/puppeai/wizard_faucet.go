...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many EtherAIs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.eaistats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.eaistats = w.readString() + ":" + w.conf.eaistats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.eaistats)
		infos.node.eaistats = w.readDefaultString(infos.node.eaistats) + ":" + w.conf.eaistats
	}
