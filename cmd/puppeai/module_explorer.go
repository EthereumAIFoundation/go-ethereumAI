...
FROM puppeai/explorer:latest

ADD eaistats.json /eaistats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../eai-net-intelligence-api && pm2 start /eaistats.json)' >  explorer.sh && \
	echo '(cd ../etheraichain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEaistats is the configuration file for the eaistats javascript client.
var explorerEaistats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.ethereumai
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Eaistats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	eaistats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEaistats)).Execute(eaistats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.eaistats[:strings.Index(config.eaistats, ":")],
		"Secret": config.eaistats[strings.Index(config.eaistats, ":")+1 : strings.Index(config.eaistats, "@")],
		"Host":   config.eaistats[strings.Index(config.eaistats, "@")+1:],
	})
	files[filepath.Join(workdir, "eaistats.json")] = eaistats.Bytes()
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Eaistats": config.eaistats[:strings.Index(config.eaistats, ":")],
	})
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Eaistats username":      info.eaistats,
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.ethereumai"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		eaistats: infos.envvars["STATS"],
	}
	return stats, nil
}
