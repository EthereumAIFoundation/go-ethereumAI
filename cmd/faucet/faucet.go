...
// faucet is a EtherAI faucet backed by a light client.
package main
...
var (
	...
	eaiPortFlag = flag.Int("eaiport", 40404, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the EthereumAI protocol")
	statsFlag   = flag.String("eaistats", "", "Eaistats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of EtherAIs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for EthereumAI and the faucet")
)

var (
	etherai = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s EtherAIs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *eaiPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // EthereumAI address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an EthereumAI light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // EthereumAI protocol stack
	client *eaiclient.Client   // Client connection to the EthereumAI chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the EthereumAI light client protocol
	...
	// Assemble the eaistats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEthereumAI
			ctx.Service(&serv)
			return eaistats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := eaiclient.NewClient(api)
...
// close terminates the EthereumAI connection and tears down the faucet.
...
// apiHandler handles requests for EtherAI grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, etherai),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the EthereumAI address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/ethereumai/go-ethereumai/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), etherai)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, etherai)
...
// the username, avatar URL and EthereumAI address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for EthereumAI addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No EthereumAI address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the EthereumAI address and profile URL.
...
// returning the username, avatar URL and EthereumAI address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain EthereumAI address,
