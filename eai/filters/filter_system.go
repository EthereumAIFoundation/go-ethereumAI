...
// Package filters implements an ethereumai filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  ethereumai.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit ethereumai.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit ethereumai.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit ethereumai.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit ethereumai.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
